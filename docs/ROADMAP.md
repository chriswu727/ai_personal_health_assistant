...
| M2: Persistent service | API, identity, database migrations, durable jobs | User isolation, concurrent revisions, restart recovery, and migration checks pass. **Partly met**: Sprint 2 delivered persistence, isolation, durable jobs, and recovery; the API and identity integration remain, in Sprint 4. |
