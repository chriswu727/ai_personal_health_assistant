...
def test_the_order_does_not_depend_on_the_order_passages_arrive_in() -> None:
    expected = [item.passage.passage_id for item in rank_passages(CORPUS, "walking sleep", at=at())]
    ...
    for _ in range(20):
        ...
        actual = [
            item.passage.passage_id for item in rank_passages(shuffled, "walking sleep", at=at())
        ]
...
def test_passages_matching_equally_well_are_ordered_by_identifier() -> None:
    ...
    results = rank_passages([second, first], "walking", at=at())
...
def test_more_occurrences_of_the_same_term_rank_higher() -> None:
    ...
    results = rank_passages([sparse, dense], "sleep", at=at())
...
def test_a_passage_that_matches_nothing_is_not_returned() -> None:
    assert rank_passages(CORPUS, "kayaking", at=at()) == ()


def test_an_empty_query_returns_nothing_rather_than_everything() -> None:
    assert rank_passages(CORPUS, "   ", at=at()) == ()
    assert rank_passages(CORPUS, "!!!", at=at()) == ()


def test_the_limit_is_respected_and_must_be_positive() -> None:
    assert len(rank_passages(CORPUS, "walking sleep energy", at=at(), limit=2)) == 2
    with pytest.raises(ValidationError):
        rank_passages(CORPUS, "walking", at=at(), limit=0)


def test_a_result_records_what_matched_and_when() -> None:
    instant = at(hours=3)
    results = rank_passages(CORPUS, "walking kayaking", at=instant)
