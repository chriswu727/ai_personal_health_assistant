...
- Runtime, dependencies, and application CI: not yet configured.
- Current work: maintainer-requested governance follow-up to Sprint 0, not an application sprint.

## Latest change

Expanded the root AGENTS.md into the mandatory AI engineering agreement. Added thin Claude, GitHub Copilot, and Cursor entry points referring to the same agreement. Added this handoff document and linked the agreement from contributor documentation. No application functionality was introduced.
...
No application tests can run at this stage. Tool-specific entry points are instruction files; they are not proof that every AI client loads or obeys them. Executable quality gates remain Sprint 1 task S1-06. These results describe the governance change, not application correctness.
